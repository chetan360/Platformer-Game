...
public class ObjectManager {
		...
		for(Potion p : potions)
			if(p.isActive()) {
				if(hitbox.intersects(p.getHitbox())) {
					p.setActive(false);
					applayEffectToPlayer(p);
				}
			}
	}
	
	public void applayEffectToPlayer(Potion p) {
	...
	private boolean isPlayerInFrountOfCannon(Cannon c, Player player) {
			...
			if(!c.doAnimation)
				if(c.getTileY() == player.getTileY())
					if(isPlayerInrange(c, player))
						if(isPlayerInFrountOfCannon(c, player))
